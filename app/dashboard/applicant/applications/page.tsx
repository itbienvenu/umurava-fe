...
export default function ApplicationsPage() {
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-[60vh]">
        <Clock size={48} weight="duotone" className="animate-pulse text-[#102C26] opacity-20 absolute" />
  ...
  return (
    <div className="max-w-5xl mx-auto pb-20">
      ...
      {error && (
        <div className="bg-red-50 text-red-600 p-4 rounded-2xl mb-8 text-sm italic border border-red-100 flex items-center gap-2 animate-in fade-in slide-in-from-top-2">
      ...
      <div className="space-y-4">
        {filteredApplications.length > 0 ? (
          filteredApplications.map((app) => (
            <div key={app._id} className="bg-white rounded-2xl border border-[#e8d0b0] p-6 hover:shadow-md transition-shadow flex flex-col md:flex-row md:items-center justify-between gap-6 group">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 bg-[#F7E7CE] rounded-2xl flex items-center justify-center text-2xl shrink-0 group-hover:scale-110 transition-transform">
                  <Buildings size={28} weight="duotone" className="text-[#102C26]" />
                </div>
                <div>
              ...
              <div className="flex flex-wrap items-center gap-6">
                ...
                <div className="flex items-center justify-between md:justify-end gap-6 w-full md:w-auto mt-4 md:mt-0 border-t md:border-t-0 pt-4 md:pt-0 border-[#fcf8f2]">
                  ...
                  <Link
                    ...
                    className="text-[#102C26] hover:underline text-sm font-bold whitespace-nowrap flex items-center gap-1 group/link"
                  >
                    View Result
          ...
          <div className="bg-white rounded-3xl border border-[#e8d0b0] p-16 flex flex-col items-center justify-center text-[#6b8f85] text-center shadow-sm">
            <FolderOpen size={64} weight="duotone" className="mb-6 opacity-30" />
